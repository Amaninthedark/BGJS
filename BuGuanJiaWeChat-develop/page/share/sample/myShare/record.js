...
Page({
   ...
   getRecord(){
      var that=this
        wx.request({
            url:url.samplesUrl+"/"+wx.getStorageSync('sampleId')+"/share/customer/record",
            method:'GET', 
            data:{
            ...
            success(res){
                if(res.data.code=="200"){
                    ...
                    for(var item of record){
                        ...
                        // item.time=timestampFormat(Date.parse(item.viewTime)/1000)
                        // item.timeSpecific=timestampFormatSpecific(Date.parse(item.viewTime)/1000)
                    }
